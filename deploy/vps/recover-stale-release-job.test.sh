...
bash "$SCRIPT_DIR/recover-stale-release-job.sh" --validate-log "$safe_log" >/dev/null
...
do
  ...
  if bash "$SCRIPT_DIR/recover-stale-release-job.sh" --validate-log "$unsafe_log" >/dev/null 2>&1; then
...
if bash "$SCRIPT_DIR/recover-stale-release-job.sh" --validate-log "$missing_stage_log" >/dev/null 2>&1; then
