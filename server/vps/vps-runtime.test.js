...
describe('IDOSI VPS runtime', () => {
  it('applies migrations once and rolls back a failed D1 batch', async () => {
    ...
    expect((await db.prepare('SELECT COUNT(*) AS count FROM _vps_migrations').first()).count).toBe(6)
    ...
    expect((await reopened.prepare('SELECT COUNT(*) AS count FROM _vps_migrations').first()).count).toBe(6)
    reopened.close()
  })
