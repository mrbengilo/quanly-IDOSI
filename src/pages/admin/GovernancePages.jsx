...
export function OrderAuditPage() {
  ...
  const auditPayload = (value) => {
    if (!value || typeof value !== 'object') return <span>—</span>
    return <div className="audit-change-list">{Object.entries(value).map(([field, fieldValue]) => <div className="audit-change-item" key={field}><strong>{AUDIT_FIELD_LABELS[field] || field}</strong><span>{field === 'amount' ? money(fieldValue) : fieldValue == null || fieldValue === '' ? '—' : typeof fieldValue === 'object' ? JSON.stringify(fieldValue) : String(fieldValue)}</span></div>)}</div>
  }
  ...
  return <div className="page">
    ...
    >
      ...
      <TableWrap><thead><tr><th>Thời gian</th><th>Đơn hàng</th><th>Cửa hàng</th><th>Thao tác</th><th>Người thực hiện</th><th>Dữ liệu trước</th><th>Dữ liệu sau</th></tr></thead><tbody>
        {rows.map((item) => <tr key={item.id}><td>{shortDateTime24(item.createdAt || item.updatedAt)}</td><td><strong>{item.orderCode || item.orderId}</strong></td><td>{storeName(item.storeId)}</td><td><Badge tone={item.action === 'Xóa' ? 'red' : item.action === 'Sửa' ? 'orange' : 'blue'}>{item.action || 'Cập nhật'}</Badge></td><td>{item.actor?.name || item.actorName || item.updatedBy || 'Hệ thống'}</td><td>{auditPayload(item.before)}</td><td>{auditPayload(item.after)}</td></tr>)}
        {!rows.length && <tr><td colSpan="7">Không có chỉnh sửa đơn hàng trong ngày {shortDate(selectedDate)} phù hợp bộ lọc.</td></tr>}
      </tbody></TableWrap>
    </Card>
  </div>
}
