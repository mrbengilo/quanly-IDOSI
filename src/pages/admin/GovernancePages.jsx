...
export function SupportTransfersPage() {
  ...
  return <div className="page governance-page"><PageHeader title="ĐIỀU CHUYỂN NHÂN SỰ" subtitle="Phân bổ nhân viên hỗ trợ giữa các cửa hàng mà không thay đổi hồ sơ gốc." icon={CalendarClock} />
    <Card title={editingId ? 'Chỉnh sửa điều chuyển' : 'Tạo điều chuyển'} className="support-transfer-card"><div className="form-grid form-grid--3">
      ...
      <Field label="Lương hỗ trợ (theo giờ)" required><MoneyInput value={form.hourlySupportRate} onChange={(event) => setForm((current) => ({ ...current, hourlySupportRate: event.target.value }))} placeholder="Nhập số nghìn" /></Field>
      <Field label="Phụ cấp"><MoneyInput value={form.allowance} onChange={(event) => setForm((current) => ({ ...current, allowance: event.target.value }))} placeholder="Nhập số nghìn" /></Field>
