...
describe('store employee current-shift orders', () => {
  ...
  it('does not reveal a coworker order through an employee order deep link', () => {
    ...
    expect(screen.getByTestId('revenue-TOTAL').textContent).toContain('120,000 đ')
    expect(screen.getByTestId('revenue-NORMAL').textContent).toContain('120,000 đ')
  ...
  it('uses the active support store for order history, attendance and new orders', async () => {
    ...
    expect(screen.getByText(/Chỉ hiển thị đơn hàng do bạn tạo trong ca đang làm tại Dosii KVC/u)).toBeTruthy()
