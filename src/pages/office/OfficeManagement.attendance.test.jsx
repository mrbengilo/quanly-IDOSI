...
describe('OfficeManagement attendance-only workspace', () => {
  ...
  it('shows full 24-hour attendance history, locations, shifts, tags, and punctuality totals', () => {
    ...
    expect(within(historyCard).getByRole('columnheader', { name: 'Vị trí' })).toBeTruthy()
    const locationLinks = within(historyCard).getAllByRole('link', { name: /Vị trí (vào|ra) của .+ trên Google Maps/i })
    expect(locationLinks).toHaveLength(4)
    locationLinks.forEach((link) => {
      expect(link.href).toMatch(/^https:\/\/www\.google\.com\/maps\/search\/\?api=1&query=/)
      expect(link.target).toBe('_blank')
      expect(link.rel).toContain('noreferrer')
      expect(['Xem vị trí vào', 'Xem vị trí ra']).toContain(link.textContent)
    })
    expect(within(historyCard).queryByText('Văn phòng tầng 1')).toBeNull()
    expect(within(historyCard).queryByText('Văn phòng tầng 2')).toBeNull()
    expect(within(historyCard).queryByText('Cổng văn phòng')).toBeNull()
    expect(within(historyCard).queryByText(/10\.80000|106\.70000/)).toBeNull()
