...
const metricValue = (label) => within(screen.getByLabelText(label === 'TỔNG DOANH THU' ? 'Doanh thu tháng của cửa hàng' : 'Tổng quan đơn hàng'))
...
const expectPendingMetrics = () => {
  ...
  expect(within(screen.getByLabelText('Doanh thu tháng của cửa hàng')).getAllByText('—')).toHaveLength(4)
}
const deferred = () => {
  let resolve
  let reject
...
describe('StoreOrdersPage remote order completeness', () => {
  it('refreshes the store history after another account creates more orders', async () => {
    ...
    expect(metrics.getAllByText('338,000 đ')).toHaveLength(1)
