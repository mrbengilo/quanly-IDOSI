...
export function StoreSettings() {
  ...
  useEffect(() => {
    // Đồng bộ biểu mẫu khi quản lý chuyển sang một cửa hàng khác.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setForm({ name: activeStore?.name || 'IDOSI', phone: activeStore?.phone || '', email: activeStore?.email || 'cuahang@idosi.vn', address: activeStore?.address || '', tax: activeStore?.tax || '', manager: activeStore?.manager || session?.name || 'Quản lý cửa hàng', opening: activeStore?.opening || '07:00', closing: activeStore?.closing || '23:00' })
  }, [activeStore?.id, activeStore?.name, activeStore?.phone, activeStore?.email, activeStore?.address, activeStore?.tax, activeStore?.manager, activeStore?.opening, activeStore?.closing, session?.name])
