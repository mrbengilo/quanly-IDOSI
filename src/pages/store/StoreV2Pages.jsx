...
export function StoreOrdersPage() {
  ...
  return (
    <div className="page store-orders-page">
      ...
      {canManageOrders && <Modal open={Boolean(editing)} onClose={() => setEditing(null)} title={`Sửa đơn hàng ${editing?.code || ''}`} footer={<><Button variant="outline" onClick={() => setEditing(null)}>Hủy</Button><Button icon={Save} onClick={save}>LƯU THAY ĐỔI</Button></>}><div className="form-grid"><Field label="Tên khách hàng"><Input value={form.customerName} onChange={(event) => setForm({ ...form, customerName: event.target.value })} /></Field><Field label="Số điện thoại"><Input value={form.customerPhone} onChange={(event) => setForm({ ...form, customerPhone: event.target.value })} /></Field><Field label="Tuổi"><Input type="number" min="0" value={form.customerAge} onChange={(event) => setForm({ ...form, customerAge: event.target.value })} /></Field><Field label="Giới tính"><Select value={form.gender} onChange={(event) => setForm({ ...form, gender: event.target.value })}><option>Nam</option><option>Nữ</option><option>Khác</option></Select></Field><Field label="Nghề nghiệp"><Input value={form.occupation} onChange={(event) => setForm({ ...form, occupation: event.target.value })} /></Field><Field label="Biết qua kênh nào"><Select value={form.acquisitionChannel} onChange={(event) => setForm({ ...form, acquisitionChannel: event.target.value })}><option>Facebook</option><option>Tiktok</option><option>Zalo</option><option>Bạn Bè</option><option>Người thân</option><option>Khác</option></Select></Field><Field label="Số tiền"><MoneyInput value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></Field><Field label="Hình thức thanh toán"><Select value={form.paymentMethod} onChange={(event) => setForm({ ...form, paymentMethod: event.target.value })}><option>Tiền mặt</option><option>Chuyển khoản</option></Select></Field><Field label="Lý do chỉnh sửa" required className="span-2"><Input value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} /></Field></div></Modal>}
    </div>
  )
}
