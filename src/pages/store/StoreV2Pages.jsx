...
import { OrderRevenueDetails, OrderRevenueEditor, OrderRevenueSummary } from '../../components/OrderRevenue'
import { orderRevenueByType, prepareOrderRevenueInput } from '../../domain/orderRevenue'
import { OrderPaymentSummary } from '../../components/OrderPaymentSummary'
import { OrderItemsSummary } from '../../components/OrderItemSelector'
...
export function StoreOrdersPage() {
  ...
  const amount = parseOrderAmountFilter(amountInput)
  const amountError = Number.isNaN(amount) ? 'Nhập số tiền nguyên không âm, ví dụ 200,000.' : ''
  const searchSettled = !remoteHistory || (query === settledSearch.query && amountInput === settledSearch.amountInput)
  const readFilters = useMemo(() => ({
    employeeId: historyEmployeeId, date, shiftId: shiftId === 'all' ? '' : shiftId,
    paymentMethod: paymentMethod === 'all' ? '' : paymentMethod,
    query: remoteHistory ? settledSearch.query : query,
    amount: parseOrderAmountFilter(remoteHistory ? settledSearch.amountInput : amountInput),
  }), [amountInput, date, historyEmployeeId, paymentMethod, query, remoteHistory, settledSearch, shiftId])
  const filterKey = JSON.stringify(readFilters)
  const hasFilters = Boolean(historyEmployeeId || date || shiftId !== 'all' || paymentMethod !== 'all' || query || amountInput)
  const historyKey = `${storeId}:${historyPeriod}:${stateVersion}:${historyRevision}:${requestedOrderParam}:${filterKey}`
  ...
  const [filteredSummary, setFilteredSummary] = useState({ key: '', value: null, error: '' })
  const filteredSummaryKey = `${summaryKey}:${filterKey}`
  ...
  const historyReady = remoteHistory && !amountError && history.key === historyKey
  ...
  useEffect(() => {
    if (!remoteHistory) return undefined
    const timer = window.setTimeout(() => setSettledSearch({ query, amountInput }), 300)
    return () => window.clearTimeout(timer)
  }, [query, amountInput, remoteHistory])
  ...
  useEffect(() => {
    if (!remoteHistory || !storeId || resolvingRequestedOrder || !hasFilters || !searchSettled || amountError) return undefined
    let active = true
    const controller = new AbortController()
    apiGetOrderSummary({ storeId, period: month, ...readFilters, signal: controller.signal }).then((payload) => {
      if (active) setFilteredSummary({ key: filteredSummaryKey, value: payload, error: '' })
    }).catch((error) => {
      if (active) setFilteredSummary({ key: filteredSummaryKey, value: null, error: error?.message || 'Không thể tải tổng kết quả lọc.' })
    })
    return () => { active = false; controller.abort() }
  }, [amountError, filteredSummaryKey, hasFilters, month, readFilters, remoteHistory, resolvingRequestedOrder, searchSettled, storeId])

  useEffect(() => {
    if (!remoteHistory || !storeId || resolvingRequestedOrder || !searchSettled || amountError) return undefined
    let active = true
    const controller = new AbortController()
    loadInitialStoreOrderHistory({
      fetchPage: (options) => apiGetHistory('orders', { ...options, signal: controller.signal }),
      query: { storeId, period: historyPeriod, ...readFilters },
      currentDay: today(),
      requestedOrderId: requestedOrderParam,
    }).then((payload) => {
      if (!active) return
      ...
      if (active) setHistory({ key: historyKey, records: [], page: null, error: error?.message || 'Không thể tải lịch sử đơn hàng.' })
    })
    return () => { active = false; controller.abort() }
  }, [amountError, historyKey, historyPeriod, readFilters, remoteHistory, requestedOrderParam, resolvingRequestedOrder, searchSettled, storeId])
  ...
  const orderEmployeeIds = new Set([...storeOrders
    ...
    ...(periodSummary?.groups?.employee || []).map((group) => compactIdentifier(group.key).toLocaleLowerCase('en-US')),
  ])
  ...
  const currentFilters = { ...readFilters, query, amount }
  const filtered = periodOrders.filter((order) => {
    if (requestedOrderKey && String(order.id) === requestedOrderKey && !hasFilters) return true
    return (!historyEmployeeId || sameOperationalIdentifier(order.employeeId, historyEmployeeId))
      && orderMatchesFilters(order, currentFilters)
  })
  const resultSummary = !hasFilters ? periodSummary
    : remoteHistory ? (searchSettled && !amountError && filteredSummary.key === filteredSummaryKey ? filteredSummary.value : null)
      : summarizeOrders(projectionOrders, { storeId, period: month, ...currentFilters })
  const resultSummaryError = hasFilters && filteredSummary.key === filteredSummaryKey ? filteredSummary.error : ''
  ...
  useEffect(() => {
    setOrderPage(1)
  }, [amountInput, date, employeeId, month, paymentMethod, query, shiftId, storeId, view])
  ...
  const openEdit = (order) => {
    setEditing(order)
    setForm({ customerName: order.customerName || '', customerPhone: order.customerPhone || '', customerAge: order.customerAge ?? '', gender: order.gender || 'Khác', occupation: order.occupation || '', acquisitionChannel: order.acquisitionChannel || 'Khác', amount: String(orderRevenueByType(order).NORMAL), paymentMethod: order.paymentMethod || '', items: Array.isArray(order.items) ? order.items : [], customFields: Array.isArray(order.customFields) ? order.customFields : [], reason: '' })
    setFormErrors({})
  }
  const closeEditor = () => {
    if (saving) return
  ...
  const save = async () => {
    ...
    const nextErrors = {}
    let revenue = { amount: editing.amount }
    if (canEditOrderAmount) {
      try { revenue = prepareOrderRevenueInput({ amount: String(form.amount).replaceAll(',', ''), items: form.items }) }
      catch (error) { nextErrors.amount = error.message }
    }
  ...
  return (
    <div className="page store-orders-page order-list-page">
      ...
      <OrderRevenueSummary totals={orderMetrics} label="Doanh thu tháng của cửa hàng" />
      ...
      <Card title="Bộ lọc đơn hàng" className="filter-card"><div className="order-filters">
        <Field label="Tìm đơn hàng"><SearchInput value={query} onChange={(value) => { setQuery(value); clearRequestedOrder() }} placeholder="Tìm mã đơn, khách hàng..." /></Field>
        <Field label="Ngày"><Input type="date" value={date} onChange={(event) => { const value = event.target.value; setDate(value); if (value) setMonth(value.slice(0, 7)); clearRequestedOrder() }} aria-label="Lọc theo ngày" /></Field>
        <Field label="Ca làm việc"><Select aria-label="Lọc theo ca" value={shiftId} onChange={(event) => { setShiftId(event.target.value); clearRequestedOrder() }}><option value="all">Tất cả ca</option>{shiftOptions.map((shift) => <option key={shift.id} value={shift.id}>{shift.name}</option>)}</Select></Field>
        <Field label="Nhân viên"><Select aria-label="Lọc theo nhân viên" value={employeeId} onChange={(event) => { setEmployeeId(event.target.value); clearRequestedOrder() }}><option value="all">Tất cả nhân viên</option>{employeeOptions.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</Select></Field>
        <Field label="Thanh toán"><Select aria-label="Lọc đơn hàng theo thanh toán" value={paymentMethod} onChange={(event) => { setPaymentMethod(event.target.value); clearRequestedOrder() }}><option value="all">Tất cả</option>{ORDER_PAYMENT_METHODS.map((method) => <option key={method} value={method}>{method}</option>)}</Select></Field>
        <Field label="Lọc số tiền chính xác" error={amountError}><Input aria-label="Lọc đơn hàng theo số tiền" inputMode="numeric" value={amountInput} placeholder="Ví dụ: 200,000" onChange={(event) => { setAmountInput(event.target.value); clearRequestedOrder() }} /></Field>
        <Button variant="outline" onClick={() => { setQuery(''); setAmountInput(''); setPaymentMethod('all'); setEmployeeId('all'); changePeriod(today().slice(0, 7)) }}>Đặt lại</Button>
      </div></Card>
      {hasFilters && !amountError && <>
        <p className="order-filter-feedback" role="status">{resultSummary ? `${resultSummary.totals.orders} đơn phù hợp${amount !== null ? ` số tiền ${money(amount)}` : ''}` : 'Đang tìm đơn hàng phù hợp...'}</p>
        {resultSummary && <OrderPaymentSummary totals={resultSummary.totals} />}
        {resultSummaryError && <InfoNote tone="red">{resultSummaryError} <Button variant="outline" onClick={() => setHistoryRevision((current) => current + 1)}>Thử lại</Button></InfoNote>}
      </>}
      ...
      {groups.map(([key, group]) => {
        ...
        const title = view === 'employee'
          ? <span className="order-group__shift-title">
              ...
              <SupportEmployeeTag record={first} employeeId={first.employeeId || first.employeeCode} storeId={storeId} businessDate={businessDate(first.createdAt || first.date)} employees={employees} stores={stores} supportTransfers={supportTransfers} />
          ...
          : view === 'day'
            ...
            : <span className="order-group__shift-title">
                ...
                <small>{first.shiftStart || '--:--'}–{first.shiftEnd || '--:--'} · {orderBusinessDate(first).split('-').reverse().join('/')}</small>
              </span>
      ...
      />
      {remoteHistory && !amountError && (!historyReady || !searchSettled) && <InfoNote>Đang tải trang lịch sử đơn hàng...</InfoNote>}
      {historyReady && history.error && <InfoNote tone="red">{history.error}</InfoNote>}
      {historyReady && history.page?.hasMore && searchSettled && (
      ...
      {!groups.length && !amountError && (!remoteHistory || (historyReady && searchSettled)) && <InfoNote>Chưa có đơn hàng phù hợp bộ lọc.</InfoNote>}
      {canEditOrders && <Modal open={Boolean(editing)} onClose={closeEditor} title={`Sửa đơn hàng ${editing?.code || ''}`} footer={<><Button variant="outline" onClick={closeEditor} disabled={saving}>Hủy</Button><Button icon={Save} loading={saving} disabled={saving} onClick={save}>LƯU THAY ĐỔI</Button></>}>
        <div className="form-grid">
          ...
          <div className="span-2"><OrderRevenueEditor options={orderInformationOptions} items={form.items} amount={form.amount} errors={formErrors} disabled={saving} canEditRevenue={canEditOrderAmount} onItemsChange={(items) => updateForm('items', items)} onAmountChange={(amount) => updateForm('amount', amount)} /></div>
...
export function StoreCashflowV2() {
  const app = useStoreData()
  const { storeId, store, stores = [], orders = [], fixedExpenses = [], employees = [], supportTransfers = [], attendance = [], addFixedExpense, notify } = app
  const [period, setPeriod] = useState(today().slice(0, 7))
  const [view, setView] = useState('shift')
      ...
      .map((row) => {
        ...
        return {
          ...
          supportRecords: [
            row.sourceOrder,
            ...relatedAttendance,
            { employeeId: row.employeeId, storeId, businessDate: row.date },
          ].filter(Boolean),
  ...
  return (
    <div className="page">
      ...
      <Card title="Doanh thu chi tiết" action={<div className="tabs tabs--compact"><button className={view === 'shift' ? 'active' : ''} onClick={() => setView('shift')}>Theo ca</button><button className={view === 'employee' ? 'active' : ''} onClick={() => setView('employee')}>Theo nhân viên</button></div>}><TableWrap><thead><tr><th>{view === 'shift' ? 'Ca làm việc' : 'Nhân viên'}</th><th>Số đơn</th><th>Tổng doanh thu</th><th>Tiền mặt</th><th>Chuyển khoản</th><th>Khác</th><th>Chi phí liên kết</th><th>Kết ca gần nhất</th></tr></thead><tbody>{groupedRevenue.map((row) => {
        ...
        const supportContext = view === 'employee' && row.employeeId
          ? resolveSupportEmployeeTagContextFromRecords({
              records: row.supportRecords,
          ...
          : null
        return <tr key={row.key}><td><strong>{label}</strong><SupportEmployeeTag context={supportContext} className="table-note" /></td><td>{row.orders}</td><td><strong>{money(row.revenue)}</strong></td><td>{money(row.cash)}</td><td>{money(row.transfer)}</td><td>{money(row.otherPayment)}</td><td>{money(row.linkedExpense)}</td><td>{checkOut}</td></tr>
...
export function StoreAttendanceV2() {
  ...
  const stats = scopedEmployees.map((employee) => {
    ...
    return { employee, sourceRecords: records, records: records.length, early, onTime, late, earlyMinutes, lateMinutes, rate, evaluation }
  ...
  return <div className="page"><PageHeader title="CHẤM CÔNG CỬA HÀNG" subtitle="Lịch sử gồm nhân viên trực thuộc và nhân viên được điều chuyển đến hỗ trợ." icon={Clock3} actions={<Input type="month" value={period} onChange={(event) => setPeriod(event.target.value)} />} /><div className="metrics-grid metrics-grid--3" aria-label="Tổng quan chấm công cửa hàng"><MetricCard label="TỔNG GIỜ GHI NHẬN" value={totalHours.toFixed(2)} suffix="giờ" icon={Clock3} tone="blue" /><MetricCard label="LƯỢT HỖ TRỢ NHẬN VÀO" value={supportRows.length} suffix="lượt" icon={Users} tone="orange" /><MetricCard label="CHI PHÍ NHÂN SỰ HỖ TRỢ" value={money(Math.floor(supportExpense))} icon={Wallet} tone="green" /></div><InfoNote>Chi phí lương hỗ trợ được ghi nhận cho <strong>{store?.name || 'cửa hàng nhận hỗ trợ'}</strong>, gồm lương theo giờ hỗ trợ và phụ cấp áp dụng một lần.</InfoNote><Card title="Lịch sử chấm công của nhân viên" action={<div className="toolbar-wrap"><SearchInput value={query} onChange={setQuery} placeholder="Tìm nhân viên..." /><Select value={shift} onChange={(event) => setShift(event.target.value)}><option value="all">Tất cả ca</option>{shifts.map(({ id, name }) => <option key={id} value={id}>{name}</option>)}</Select><Select value={employeeId} onChange={(event) => setEmployeeId(event.target.value)}><option value="all">Tất cả nhân viên</option>{scopedEmployees.map((employee) => <option key={employeePrimaryIdentifier(employee)} value={employeePrimaryIdentifier(employee)}>{employee.name}</option>)}</Select></div>}><TableWrap firstPageDates={[today()]} paginationKey={`${period}:${query}:${shift}:${employeeId}`}><thead><tr><th>STT</th><th>Nhân viên / Cửa hàng</th><th>Ca làm việc</th><th>Ngày</th><th>Giờ vào / Kết</th><th>Giờ thực tế</th><th>Trạng thái</th><th>Phút sớm / trễ</th><th>Phụ cấp TikTok</th><th>Lương thực nhận</th><th>Vị trí</th></tr></thead><tbody>{rowDetails.map(({ record, employee, pay }, index) => {
    ...
    const supportTime = pay.support?.transferStartAt || pay.support?.transferEndAt
      ? formatVietnamTransferPeriod({
          startAt: pay.support.transferStartAt,
          endAt: pay.support.transferEndAt,
        })
      : ''
  ...
  })}<tr className="total-row"><td colSpan="5">TỔNG</td><td>{totalHours.toFixed(2)} giờ</td><td colSpan="2" /><td>{money(totalTiktok)}</td><td>{money(Math.floor(totalPay))}</td><td /></tr></tbody></TableWrap></Card><Card title="THỐNG KÊ ĐI LÀM ĐÚNG GIỜ"><TableWrap><thead><tr><th>STT</th><th>Nhân viên</th><th>Đi trễ</th><th>Đi đúng giờ</th><th>Đi sớm</th><th>Tổng phút sớm</th><th>Tổng phút trễ</th><th>Tổng ca</th><th>Tỷ lệ đúng giờ</th><th>Đánh giá</th></tr></thead><tbody>{stats.map((item, index) => <tr key={item.employee.id}><td>{index + 1}</td><td><strong>{item.employee.name}</strong><SupportEmployeeTag context={resolveSupportEmployeeTagContextFromRecords({ records: item.sourceRecords, employee: item.employee, employeeId: item.employee.id, storeId, employees, stores, supportTransfers })} className="table-note" /><small className="table-note">{item.employee.id}</small></td><td className="red-text attendance-stat-value">{item.late}</td><td className="blue-text attendance-stat-value">{item.onTime}</td><td className="green-text attendance-stat-value">{item.early}</td><td className="green-text attendance-stat-value">{item.earlyMinutes}</td><td className="red-text attendance-stat-value">{item.lateMinutes}</td><td>{item.records}</td><td className="blue-text attendance-stat-value"><strong>{item.rate.toFixed(1)}%</strong></td><td><Badge tone={item.evaluation === 'Chuyên cần tốt' ? 'green' : item.evaluation === 'Cần cải thiện' ? 'red' : 'orange'}>{item.evaluation}</Badge></td></tr>)}</tbody></TableWrap></Card></div>
}
        ...
        <TableWrap><thead><tr><th>Thời gian</th><th>Nhân viên</th><th>Số tiền ứng</th><th>Lương khả dụng lúc tạo</th><th>Còn lại</th><th>Người tạo</th><th>Ghi chú</th><th>Trạng thái</th>{canOperatePayroll && <th>Hành động</th>}</tr></thead><tbody>
          {scopedSalaryAdvances.map((item) => {
            const availability = item.status === 'Mới tạo'
              ...
              : { ok: true }
            return <tr key={item.id}><td>{timestamp(item.createdAt)}</td><td><strong>{item.employeeName}</strong><SupportEmployeeTag record={item} employeeId={item.employeeId || item.employeeCode} storeId={storeId} businessDate={item.createdAt || `${period}-01`} employees={employees} stores={stores} supportTransfers={supportTransfers} className="table-note" /><small className="table-note">{item.employeeId}</small></td><td>{money(item.amount)}</td><td>{money(item.availableAtCreation)}</td><td>{money(item.remainingAfter)}</td><td>{item.createdBy?.name}</td><td>{item.note || '—'}</td><td><Badge tone={item.status === 'Đã chi' ? 'green' : 'orange'}>{item.status}</Badge></td>{canOperatePayroll && <td>{item.status === 'Mới tạo' ? <Button icon={CheckCircle2} disabled={!availability.ok || Boolean(payrollPreviewError)} title={!availability.ok ? availability.message : undefined} onClick={() => handleConfirmAdvance(item.id)}>XÁC NHẬN CHI</Button> : <span>{timestamp(item.confirmedAt)}</span>}</td>}</tr>
          })}
