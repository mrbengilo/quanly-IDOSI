...
describe('store tiered payroll', () => {
  it('classifies only explicit operational Dosii and SM store identities', () => {
    ...
    expect(classifyStorePayrollPolicy({ id: 'SM-TNV', name: 'Cửa hàng trung tâm' })).toBe(STORE_PAYROLL_POLICY.SM_TNV)
    expect(classifyStorePayrollPolicy({ id: 'STORE-01', code: '\u00a0sm\u2011tnv\u2007', name: 'Cửa hàng trung tâm' })).toBe(STORE_PAYROLL_POLICY.SM_TNV)
    expect(classifyStorePayrollPolicy({ id: 'LEGACY', name: 'SecondMall SM234' })).toBe(STORE_PAYROLL_POLICY.SM_TNV)
    expect(classifyStorePayrollPolicy({ id: 'OTHER', code: 'ASM-TNV-X' })).toBe(STORE_PAYROLL_POLICY.UNSUPPORTED)
    expect(classifyStorePayrollPolicy({ id: 'OTHER', code: 'XSM234Y' })).toBe(STORE_PAYROLL_POLICY.UNSUPPORTED)
