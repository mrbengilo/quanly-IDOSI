...
describe('orderSummary', () => {
  ...
  it('summarizes the full eligible scoped period by payment method and group', () => {
    ...
    expect(result.totals).toEqual({ orders: 3, cash: 100_000, transfer: 250_000, revenue: 400_000, cashOrders: 1, transferOrders: 1, revenueByType: { NORMAL: 400_000, SALE_KG: 0, SALE_PIECE: 0 }, totalSaleRevenue: 0 })
  ...
  it('preserves valid legacy numeric-string VND amounts', () => {
    expect(summarizeOrders([
      ...
      revenueByType: { NORMAL: 125_000, SALE_KG: 0, SALE_PIECE: 0 }, totalSaleRevenue: 0,
    })
  })
  ...
  it('combines exact amounts, payment aliases, Vietnam dates and text without changing whole-shift totals', () => {
    ...
    expect(summarizeOrders(rows, scope).totals).toEqual({ orders: 4, revenue: 80_001, revenueByType: { NORMAL: 80_001, SALE_KG: 0, SALE_PIECE: 0 }, totalSaleRevenue: 0, cash: 40_001, transfer: 20_000, cashOrders: 2, transferOrders: 1 })
    const result = summarizeOrders(rows, { ...scope, amount: 20_000, paymentMethod: 'cash', date: '2026-09-01', shiftId: 'night', query: 'ÁNH' })
    expect(result.totals).toEqual({ orders: 1, revenue: 20_000, revenueByType: { NORMAL: 20_000, SALE_KG: 0, SALE_PIECE: 0 }, totalSaleRevenue: 0, cash: 20_000, transfer: 0, cashOrders: 1, transferOrders: 0 })
