...
export const CUSTOMER_OCCUPATIONS = DEFAULT_OCCUPATION_LABELS
...
export function customerSurveySummary(orders = [], filters = {}) {
  ...
  const configuredOccupations = new Set(occupationOptions(filters.occupationOptions, { includeInactive: true })
  ...
  const occupationRecords = records.filter((order) => {
    ...
    return Number.isFinite(timestamp)
      ...
      && configuredOccupations.has(String(order.occupation || '').trim())
  })
